...
const FileUpload = ({ files, errors, touched, setFieldValue }) => {
  ...
  const handleRemoveFile = filePath => {
    const newStateFiles = files.filter(file => file.path !== filePath)
  ...
  return (
    <>
     <Typography component="h6" variant="h6" color={errors && touched ? 'error' : 'textPrimary'}>
      ...
      {
        errors && touched
        ? <Typography variant="body2" color={errors && touched ? 'error' : 'textPrimary'} gutterBottom>{errors}</Typography>
        : null
      }
